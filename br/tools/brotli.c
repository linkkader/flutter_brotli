...
#include "../include/brotli/brotli.h"
#include <errno.h>
#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
...
static BROTLI_BOOL OpenInputFile(const char* input_path, FILE** f,  int in) {
  *f = NULL;
  if (!input_path) {
    *f = fdopen(MAKE_BINARY(in), "rb");
    return BROTLI_TRUE;
  }
...
static BROTLI_BOOL OpenFiles(Context* context) {
  BROTLI_BOOL is_ok = OpenInputFile(context->current_input_path, &context->fin, context->in);
...
int main2(int fd, char *out, int is_compress) {
  Command command;
  Context context;
  BROTLI_BOOL is_ok = BROTLI_TRUE;
  int i;
  ...
  switch (command) {
    ...
    case COMMAND_COMPRESS:
	  ...
	  printf("%ld", context.next_out - context.output);
      break;
...
TVALUE *myCompress(char *str, char *path)
{
	...
	in_path = STRJOIN(path, "in");
	out_path = STRJOIN(path, "out");
